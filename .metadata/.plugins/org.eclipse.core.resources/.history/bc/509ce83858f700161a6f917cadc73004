...
public class SimulinkBlockWrapper {
  ...
  public String getSignalDeclaration() {
    ...
    String outDataTypeStr = this.getDeclaredParameter("OutDataTypeStr");
    if (Util.stringNullOrEmpty(outDataTypeStr)) {
      outDataTypeStr = "double";
    }
    if (outDataTypeStr.equals("boolean")) {
      signalDeclaration = String.format("bool %s;", getSignalName());
    }
